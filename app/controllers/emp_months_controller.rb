  ...
  def new
    ...
    (1..(1.month.from_now.end_of_month.strftime("%d")).to_i).each do |date| # looping all next month's days
      ...
      Employee.where("grade IN (1,2)").order("RANDOM()").each do |emp| # looping all employees in random order
        ...
        if "#{Date.today.strftime('%Y')}-#{1.month.from_now.strftime("%m")}-#{date}".to_date.sunday? # checking day
          if emp.grade.to_i==1 && emp.total_leave_day.to_i<4 && @emp_date.total_top_grade.to_i==0 && @emp_date.total_emp.to_i<3 && date.to_i-emp.last_leave_day.to_i>5
          ...
          elsif emp.grade.to_i==2 && emp.total_leave_day.to_i<4 && @emp_date.total_emp.to_i<3 && date.to_i-emp.last_leave_day.to_i>5
            if emp.gender.to_i==2 && @emp_date.female_emp.to_i==0
